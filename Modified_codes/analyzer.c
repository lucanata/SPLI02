...
//L'utilizzo è ./analyzer general.conf.txt

 //Controllo se ho 2 argomenti
  if(argc!=2){
    fprintf(stderr,"Use %s config_file\n",argv[0]);
    exit(1);
  }
  fp=fopen(argv[1],"rt");								//Apertura file configurazione
  if(fp==NULL)exit(1);									//Controllo che ci sia il file altrimenti exit
  for(;;){												//ciclo dove leggo il file e inizializzo i valori delle variabili
    fscanf(fp,"%s",buffer);
